...
{
    public class SobolevSpectralOdeOperator : ISpectralOdeOperator<double[][]>
    {
        ...
        private readonly IEnumerable<RealFunction> _phi;
        private readonly IEnumerable<RealFunction> _phiSobolev;
        ...
        private Segment _OrthogonalitySegment;
        public Segment OrthogonalitySegment {
            get {
                return _OrthogonalitySegment;
            }
            set {
                _OrthogonalitySegment = value;
            }
        }



        //public SobolevSpectralOdeOperator(IEnumerable<Func<double, double>> phi, IEnumerable<Func<double, double>> phiSobolev, double[] quadratureNodes)
        public SobolevSpectralOdeOperator(double[] quadratureNodes, FunctionsSystem phi, FunctionsSystem phiSobolev, bool weighted = false)
        {
            if (weighted) {
                _phi = Natural.NumbersWithZero.Select(k => phi.GetWeighted(k));
            } else {
                _phi = Natural.NumbersWithZero.Select(k => phi.Get(k));
            }
            _phiSobolev = Natural.NumbersWithZero.Select(k => phiSobolev.Get(k));
            _nodes = quadratureNodes;

            OrthogonalitySegment = phi.OrthogonalitySegment;
        ...
        private double[] CalcEta(int k, double[] c)
        {
            return _nodes
                .Select(t => 
                    _initialValues[k] + c.Zip(_phiSobolevCached.Skip(1), (ci, phiiPlus1) => ci * phiiPlus1(t)).Sum()
                )
                .ToArray();
        }
